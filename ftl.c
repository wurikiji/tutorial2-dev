...
static void init_smt_blk_curs(void);
static void logging_smt_cache(void);
static void write_smt_bundle(UINT32 const b_index);     // modified by RED
static void load_smt_bundle(UINT32 const b_index);      // modified by RED
static UINT32 get_psn(UINT32 const lba);
static void set_psn(UINT32 const lba, UINT32 const psn);
static UINT32 is_bundle_in_cache(UINT32 const bundle);
...
typedef struct _misc_metadata
{
	...
	UINT32 g_target_row;
	UINT32 g_smt_blk_curs[TOTAL_SMT_BNDLS];      // modified by RED
}misc_metadata; // per bank

// User-defined Macro
#define MISCBLK_VBN	1
#define EMPTY_SMT_BNDL     (UINT32)0xFFFFFFFF
...
static UINT32 g_dirty_bundles[SMT_CACHE_BNDLS];     // modified by RED
//----------------------------------

//Macro for accessing global variables
//Modified by RED
#define get_smt_vblk(bundle, bank)          (g_misc_meta[bank].g_smt_blk_curs[bundle] / PAGES_PER_VBLK)
#define set_smt_vblk(bundle, bank, vblk)    (g_misc_meta[bank].g_smt_blk_curs[bundle] = vblk * PAGES_PER_VBLK)
#define get_smt_blk_cur(bundle, bank)       (g_misc_meta[bank].g_smt_blk_curs[bundle])
#define reset_smt_blk_cur(bundle, bank)     (g_misc_meta[bank].g_smt_blk_curs[bundle] = g_misc_meta[bank].g_smt_blk_curs[bundle] / PAGES_PER_VBLK * PAGES_PER_VBLK)
#define inc_smt_blk_cur(bundle, bank)       (g_misc_meta[bank].g_smt_blk_curs[bundle]++)
#define get_dirty_bundle(index)             (g_dirty_bundles[index])
#define set_dirty_bundle(index, bundle)     (g_dirty_bundles[index] = bundle)
#define get_smt_bundle(lba)                 (lba / ELMTS_PER_BNDL)
#define get_smt_piece(lba)                  ((lba % ELMTS_PER_BNDL) / ELMTS_PER_PIECE)
...
void ftl_open(void)
{
    ...
    // STEP 2 - initialize metadata before format or loading
	...
	{
		// if there is format mark, load misc metadata from NAND.
		loading_misc_metadata();
	}

	//*Red//
	// STEP 4 - initialize sector mapping table pieces
	// The page mapping table is too large to fit in SRAM and DRAM.
    mem_set_dram(SMT_CACHE_ADDR, NULL, SMT_CACHE_BYTES);
    {
        UINT32 b_index;
        for (b_index = 0; b_index < SMT_CACHE_BNDLS; b_index++) {
            // set as empty piece.
            set_dirty_bundle(b_index,EMPTY_SMT_BNDL);
        }
    }
    
	// STEP 5 - initialize global variables that belong to FTL
...
void ftl_read_sector(UINT32 const lba, UINT32 const sect_offset)
{
...
static void init_smt_blk_curs(void)     // modified by RED
{
    ...
    for (bank = 0; bank < NUM_BANKS; bank++) {
        ...
        do {
            if (is_bad_block(bank, vblock) == FALSE) {
                set_smt_vblk(bundle, bank, vblock);
                bundle++;
            }
            vblock++;
        } while (bundle < TOTAL_SMT_BNDLS && vblock < VBLKS_PER_BANK);
    }
    ASSERT(vblock < VBLKS_PER_BANK);
}
static void logging_smt_cache(void)     // modified by RED
{
    UINT32 b_index, bundle, bank, piece, vblock;
    UINT32 cache_addr;
    UINT32 vpn;
    
    // for each piece each bank, if the block is full, erase SMT block.
    for (b_index = 0; b_index < SMT_CACHE_BNDLS; b_index++) {
        // get the present bundle number.
        bundle = get_dirty_bundle(b_index);
        
        // if there is no bundle, skip.
        if (bundle == EMPTY_SMT_BNDL)
            continue;
        
        // if the block is full, erase SMT block.
        if (get_smt_blk_cur(bundle, 0) % PAGES_PER_VBLK == PAGES_PER_VBLK - PIECES_PER_BNDL) {
            for (bank = 0; bank < NUM_BANKS; bank++) {            
                // Get vblock.
                vblock = get_smt_vblk(bundle, bank);
    ...
    // for each SMT bundle on DRAM,
    for (b_index = 0; b_index < SMT_CACHE_BNDLS; b_index++) {
        // get the present bundle number
        bundle = get_dirty_bundle(b_index);
        
        // if there is no bundle, skip.
        if (piece == EMPTY_SMT_BNDL)
            continue;
        
        // for each SMT piece in a bundle,
        for (piece = 0; piece < PIECES_PER_BNDL; piece++) {
            // wait for finish of flash behavior
            flash_finish();
            
            // copy SMT page on DRAM to FTL buffer
            for (bank = 0; bank < NUM_BANKS; bank++) {
                cache_addr = SMT_CACHE_ADDR + BYTES_PER_BNDL * b_index + BYTES_PER_PIECE * piece + BYTES_PER_PAGE * bank;
                mem_copy(FTL_BUF_ADDR+((bank)*BYTES_PER_PAGE), cache_addr, BYTES_PER_PAGE);
            }
            
            // write SMT page on FTL buffer to NAND
            for (bank = 0; bank < NUM_BANKS; bank++) {
                // get vpn of the page to be written.
                vpn = get_smt_blk_cur(bundle, bank);
                ...
                // increase SMT block cursor.
                inc_smt_blk_cur(bundle, bank);
            }
        }
    }
...
static void write_smt_bundle(UINT32 const b_index)  // modified by RED
{
    UINT32 bundle, bank, piece, vblock, cache_addr, vpn;
    
    // get the present bundle number
    bundle = get_dirty_bundle(b_index);
    
    // if there is no bundle, exit this function.
    if (bundle == EMPTY_SMT_BNDL) {
        return;
    }
...
static void load_smt_bundle(UINT32 const b_index)    // modified by RED
{
    UINT32 bundle, piece, vpn, cache_addr,bank;
    
    // get the present bundle number
    bundle = get_dirty_bundle(b_index);
    
    // if there is any bundle, exit this function.
    if (bundle != EMPTY_SMT_BNDL) {
        return;
    }
    ...
    for (piece = 0; piece < PIECES_PER_BNDL; piece++) {
        ...
        for (bank = 0; bank < NUM_BANKS; bank++) {
            ...
            cache_addr = SMT_CACHE_ADDR + BYTES_PER_PIECE * b_index + BYTES_PER_PAGE * bank;
            mem_copy(cache_addr, FTL_BUF_ADDR + ((bank) * BYTES_PER_PAGE), BYTES_PER_PAGE);
        }
    }
    
    // check a flag that the piece exists.
    set_dirty_bundle(b_index, bundle);
}

static UINT32 is_bundle_in_cache(UINT32 const bundle)   // modified by RED
{
    // check if there is corresponding bundle on DRAM.
    UINT32 b_index;
    for (b_index = 0; b_index < SMT_CACHE_BNDLS; b_index++) {
        if (get_dirty_bundle(b_index) == bundle) {
            return TRUE;
        }
    }
    return FALSE;
...
{   
    UINT32 offset, piece, bank, bundle;
    ...
    if(is_bundle_in_cache(bundle) == FALSE) {
        // SWAP area
        // In one bank mode, it is not needed.
        write_smt_bundle(bundle);
        load_smt_bundle(bundle);
    }
    
    // Get PSN from the SMT piece.
    return read_dram_32(SMT_CACHE_ADDR + bundle * BYTES_PER_BNDL + piece * BYTES_PER_PIECE + bank * BYTES_PER_PAGE + offset * sizeof(UINT32));
}
static void set_psn(UINT32 const lba, UINT32 const psn)			//modified by RED
{
    ...
    write_dram_32(SMT_CACHE_ADDR + bundle * BYTES_PER_BNDL + piece * BYTES_PER_PIECE + bank * BYTES_PER_PAGE + offset * sizeof(UINT32), psn);
}
static UINT32 get_free_page(UINT32 const bank)
{
