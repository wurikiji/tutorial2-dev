...
//defined by user.
#define PIECES_PER_BUNDLE   (PAGES_PER_VBLK / 8)  // bundle size
#define BUNDLES_ON_CACHE    (NUM_BUNDLES)  // the number of bundles on cache (In 2GB SSD, all bundles could be on DRAM)

#define NUM_BUNDLES         ((SECTORS_PER_PAGE * PAGES_PER_VBLK * VBLKS_PER_BANK * sizeof(UINT32) + BYTES_PER_BUNDLE - 1) / BYTES_PER_BUNDLE)
#define BYTES_PER_BUNDLE    (BYTES_PER_PIECE * PIECES_PER_BUNDLE)
#define BYTES_PER_PIECE     (BYTES_PER_PAGE * NUM_BANKS)

#define ELMTS_PER_BUNDLE    (BYTES_PER_BUNDLE / sizeof(UINT32)) // SMT elements per a bundle.
