...
void ftl_open(void)
{
    ...
    mem_set_dram(CACHED_SMT_ADDR, NULL, CACHED_SMT_ADDR);
    {
        UINT32 b_index;
...
static void init_smt_metadata(void)     // modified by RED
{
    ...
    for (bank = 0; bank < NUM_BANKS; bank++)
    {
        ...
        while (bundle < NUM_BUNDLES && vblock < VBLKS_PER_BANK)
        {
            ...
            {
                set_bundle_map_vpn(bundle, bank, vblock);
                bundle++;
            }
            vblock++;
        }
...
static void logging_smt_cache(void)     // modified by RED
{
    ...
    for (b_index = 0; b_index < BUNDLES_ON_CACHE; b_index++) 
    {
        ...
        bundle = get_bundle_on_cache(b_index);
        
        // if the bundle is empty, skip logging it.
        if (bundle == EMPTY_SMT_BUNDLE)
            continue;
        
        // get the vpn of the bundle's first bank.
        vpn = get_bundle_map_vpn(bundle, 0);
        
        // check if there is sufficient space for logging SMT cache.
        if (vpn % PAGES_PER_VBLK == PAGES_PER_VBLK - PIECES_PER_BUNDLE) 
        {
            // If there is no sufficient space, erase a bundle.
            for (bank = 0; bank < NUM_BANKS; bank++) 
            {            
                // Get vblock.
        ...
        for (piece = 0; piece < PIECES_PER_BUNDLE; piece++) 
        {
            ...
            for (bank = 0; bank < NUM_BANKS; bank++) 
            {
                ...
                inc_bundle_map_vpn(bundle, bank);
            }
...
static void evict_smt_bundle(UINT32 const b_index)  // modified by RED
{
    ...
    // the bundle space on cache must not empty.
    ASSERT(bundle != EMPTY_SMT_BUNDLE)
...
static void fetch_smt_bundle(UINT32 const b_index, UINT32 const bundle)    // modified by RED
{
    ...
    // the bundle space on cache must empty.
    ASSERT(bundle == EMPTY_SMT_BUNDLE);
...
static UINT32 get_psn(UINT32 const lba)		//modified by RED
{
    ...
    if(b_index == NOT_EXIST)
    {
        ...
        // if there is no bundle on DRAM, eviction function is not needed.
        // but else, victim SMT bundle must be evicted.
        if (b_index != NOT_EXIST)
