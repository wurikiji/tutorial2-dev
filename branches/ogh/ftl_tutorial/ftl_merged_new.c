...
void ftl_open(void)
{
    ...
    {
        UINT32 b_index;
        for (b_index = 0; b_index < SMT_CACHE_BNDLS; p++) {
            // set as empty piece.
            set_dirty_bundle(EMPTY_SMT_BNDL);
        }
    }
...
static void load_smt_bundle(UINT32 const b_index)    // modified by RED
{
    UINT32 bundle, piece, vpn, cache_addr;
...
{   
    ...
    if(is_bundle_in_cache(bundle) == FALSE) {
        ...
        write_smt_piece(bundle);
        load_smt_piece(bundle);
    }
    
    // Get PSN from the SMT piece.
    return read_dram_32(SMT_PIECES_ADDR + bundle * BYTES_PER_BNDL + piece * BYTES_PER_PIECE + bank * BYTES_PER_PAGE + offset * sizeof(UINT32));
}
static void set_psn(UINT32 const lba, UINT32 const psn)			//modified by RED
{
    ...
    write_dram_32(SMT_PIECES_ADDR + bundle * BYTES_PER_BNDL + piece * BYTES_PER_PIECE + bank * BYTES_PER_PAGE + offset * sizeof(UINT32), psn);
}
static UINT32 get_free_page(UINT32 const bank)
{
