...
//static void ftl_read2(UINT32 const lba, UINT32 const total_sectors);				//modified by GYUHWA
	//Added by GYUHWA
...
void ftl_read(UINT32 const lba, UINT32 const total_sectors)				//modified by GYUHWA
{
	...
	for(count = 0; count < total_sectors; count++)
	{
		...
		if(sect_offset == SECTORS_PER_PAGE )				//One page is read complete
		{
			...
			sect_offset = 0;
			get_psn(lba+count, SECTORS_PER_PAGE);
			g_psn_sect = 0;
		}
	}
...
void ftl_write_sector(UINT32 const lba)
{
	...
	if( (temp & (UINT32)BIT31) != 0 ){
		...
		g_psn_write[g_psn_write_sect++] = ((UINT32)BIT31 ^ vsect_num );
...
{   
	...
	//read_dram_32(SMT_PIECES_ADDR + (head * (SECTS_PER_SMT_PIECE * NUM_BANKS) + offset * (NUM_BANKS) + bank) * sizeof(UINT32));
	mem_copy(g_psn_read,
			SMT_PIECES_ADDR + (head * (SECTS_PER_SMT_PIECE * NUM_BANKS) + offset * (NUM_BANKS) + bank) * sizeof(UINT32),
			sizeof(UINT32) * totals);
}
static void set_psn(UINT32 const lba, UINT32 const totals)			//added by RED
{
	...
	mem_copy(SMT_PIECES_ADDR + (head * (SECTS_PER_SMT_PIECE * NUM_BANKS) + offset * (NUM_BANKS) + bank) * sizeof(UINT32),
			g_psn_write + (sizeof(UINT32) * g_psn_write_temp),
			sizeof(UINT32) * totals);
}
static UINT32 get_free_page(UINT32 const bank)
{
